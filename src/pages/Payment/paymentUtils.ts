...
const initializePayment = async ({
  ...
  const cleanup = async () => {
    ...
    try {
      if (currentWidgets && typeof currentWidgets.destroy === 'function') {
  ...
  const initializeWidget = async () => {
    try {
      ...
      // 세션 타임아웃 체크를 위한 타이머 설정
      if (refreshTimeoutId) {
        clearTimeout(refreshTimeoutId);
      }

      refreshTimeoutId = setTimeout(() => {
        if (isMounted && sessionStartTime) {
      ...
      if (isMounted) {
        ...
        navigate(PAYMENT_PATHS.FAIL, {
          state: { error: error instanceof Error ? error.message : t('error.unknown') },
        });
      }
    }
  };
